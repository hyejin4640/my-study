...
const arrFront = [];
let totalScroll = 0;
const zSecPos =[-500,0,200,400,500];  /* box의 transform : transitionZ값의 구간을 담아놓은 배열 */
...
const setSectionPos =()=>{
  ...
  for( let i=1; i<=tagFront.length; i++){
    arrFront[tagFront.length-i] = endPos;   /* 3 2 1 0 :배열에 거꾸로 저장 */
...
const handlerScroll =()=>{
  ...
  const zPos = percent *970 + END_POSITION; /* 선택값 * 전체값비율 => 전체값에 선택값이 차지하는 비율을 계산하는식 */
  console.log("z", percent, zPos );
