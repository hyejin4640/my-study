...
const handlerClickNext = (e) => {
  currentIdx++;
  if (currentIdx <= SLIDE_LEN ) {  // 5 보다 cidx가 작거나 같을때 
    $slideList.style.transition = MOVE_TIME+"ms";  // 300ms 씩 움직여라
    $slideList.style.transform = `translateX(-${ IMG_WIDTH * (currentIdx + 1)}px)`;  //-300* (1,2,3,4,5) 
  }
  if (currentIdx === SLIDE_LEN ) {  //5와 cidx가 같을때
    // currentIdx = SLIDE_LEN-1 ;
    setTimeout(() => { 
